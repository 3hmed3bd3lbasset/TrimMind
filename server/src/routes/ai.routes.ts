...
router.post('/chat', aiLimiter, optionalAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    let cleanPushName = (pushName || '').replace(/[^\p{L}\s]/gu, '').trim();
    if (cleanPushName.length > 25) cleanPushName = cleanPushName.slice(0, 25);
...
🚫 قاعدة صارمة جداً وحاسمة بخصوص التحية والترحيب:
${isContinuingConversation ? `
- هذه محادثة جارية ومستمرة (ليست أول رسالة).
- ❌ ممنوع منعاً باتاً وأكيداً كتابة أي تحية أو ترحيب أو ديباجة افتتاحية (ممنوع تماماً: "أهلاً بك", "يا هلا", "نورتنا", "تحت أمرك", "منور صالون...", "يا فندم نورتنا", "ولا يهمك يا فندم", أو تكرار اسم العميل كافتتاحية).
- ادخل في الرد أو الإجراء فوراً ومباشرة بدون أي مقدمات!` : `
- هذه أول رسالة في المحادثة، رحب بالعميل باختصار ولطافة في جملة واحدة فقط ثم اسأله عن رغبته.`}

🧠 قواعد ذكية لتتبع الحجز وعدم النسيان (Smart Booking State Continuity):
1. راجع كل رسائل المحادثة السابقة بدقة واحتفظ بما تم التوصل إليه:
   - إذا كنت قد اقترحت على العميل خدمة معينة (مثل "VIP Full Experience") أو كابتن معين (مثل "كابتن محمد الحداد")، وقال العميل: "تمام", "احجزلي", "ماشي", أو حدد موعداً (مثل "بكرا الساعة 5 العصر") ⬅️ فهذا يعني موافقة العميل الكاملة على الخدمة والكابتن المقترحين!
   - ❌ إياك ثم إياك أن تسأله مرة ثانية: "تحب تختار خدمة إيه؟" أو "تحب تحجز مع أنهي كابتن؟".
   - الإجراء الصحيح: أكد له الحجز فوراً بهذه البيانات المعتمدة:
     • الخدمة: (الخدمة المقترحة والمعتمدة)
     • الكابتن: (الكابتن المقترح والمعتمد)
     • الموعد: (الموعد المطلوب)
     • العربون المطلوب: (${deposits.vip} ج.م للخدمات VIP أو ${deposits.normal} ج.م للخدمات العادية)
     واطلب منه تأكيد رغبته لإتمام الحجز.

2. إذا سأل العميل أو أجاب باختصار (مثل: "أي واحد", "اختارلي أنت", "تمام"):
...
${customContext ? `\nسياق إضافي: ${String(customContext).slice(0, 500)}` : ''}`;
    ...
    if (!responseText) {
      ...
      responseText = isContinuingConversation
        ? `تحت أمرك، تم تسجيل طلبك بخصوص باقاتنا الملكية. تحب نأكد الحجز وموعد الجلسة الآن؟`
    ...
    if (isContinuingConversation && responseText) {
      responseText = responseText
        .replace(/^(أهلاً بحضرتك|أهلاً بك|يا هلا بيك|يا هلا بحضرتك|يا هلا يا فندم|يا هلا بك|نورتنا|منورنا دايماً|منورنا|تحت أمرك يا فندم|ولا يهمك يا فندم)[^\n.!?]*[\n.!?]\s*/i, '')
        .trim();
    }
